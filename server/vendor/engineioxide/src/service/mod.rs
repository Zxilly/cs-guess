...
where
    H: EngineIoHandler,
{
    /// Create a new engine.io connection over an already-upgraded WebSocket
    /// byte stream. Mostly used for testing.
    pub fn ws_init<S>(
        &self,
        conn: S,
        ...
        S: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin + Send + 'static,
    {
        ...
        let websocket = crate::transport::ws::raw_websocket(conn, &engine.config);
        crate::transport::ws::on_init(engine, websocket, protocol, sid, req_data)
    }
}
