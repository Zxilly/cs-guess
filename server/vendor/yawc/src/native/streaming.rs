    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    pub(crate) fn new(role: Role, stream: S, read_buf: Bytes, negotiated: &Negotiation) -> Self {
        ...
        Self {
            ...
            compression_threshold: negotiated.compression_threshold,
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    fn start_send(self: Pin<&mut Self>, mut item: Frame) -> std::result::Result<(), Self::Error> {
        ...
        let should_compress =
            !item.opcode.is_control() && item.payload.len() >= this.compression_threshold;
        if should_compress {
...
mod compression_threshold_tests {
    ...
    async fn first_wire_byte(payload_size: usize, threshold: usize) -> u8 {
        let (server, mut client) = tokio::io::duplex(payload_size.saturating_mul(2).max(64));
        let negotiation = Negotiation {
            extensions: Some(WebSocketExtensions::default()),
            compression_level: Some(CompressionLevel::fast()),
            compression_threshold: threshold,
            max_payload_read: payload_size.saturating_mul(2),
            max_read_buffer: payload_size.saturating_mul(2),
            ..Default::default()
        };
        let mut websocket = Streaming::new(Role::Server, server, Bytes::new(), &negotiation);
        websocket
            .send(Frame::binary(vec![b'a'; payload_size]))
            .await
    ...
    #[tokio::test]
    async fn only_compresses_payloads_at_or_above_the_threshold() {
        let small = first_wire_byte(64, 1024).await;
        let large = first_wire_byte(2048, 1024).await;

        assert_eq!(small & 0x40, 0, "small frame unexpectedly set RSV1");
        assert_eq!(large & 0x40, 0x40, "large frame did not set RSV1");
    }
}
