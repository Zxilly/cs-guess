...
{
    /// Creates a WebSocket over an already-upgraded byte stream.
    ///
    /// Compression is only active when explicitly enabled in `options`;
    /// callers are responsible for negotiating matching extension parameters
    /// with the peer.
    pub fn from_raw_socket(role: Role, stream: S, options: Options) -> Self {
        let max_read_buffer = options.max_read_buffer.unwrap_or(
            options
                .max_payload_read
                .map(|payload_read| payload_read * 2)
        ...
        let extensions = options.compression.as_ref().map(WebSocketExtensions::from);
        ...
        Self::new(role, stream, Bytes::new(), negotiation)
    }
