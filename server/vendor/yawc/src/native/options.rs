...
pub struct Options {
    ...
    /// Minimum outgoing data-frame payload size that will be compressed.
    ///
    /// Smaller messages keep their original representation, avoiding Deflate
    /// framing overhead and CPU work when compression cannot provide a useful
    /// reduction. Defaults to `0` to preserve the upstream behavior.
...
impl Options {
    /// Only compress outgoing data frames whose payload is at least `size`
    /// bytes. Control frames are never compressed.
    pub fn with_compression_threshold(self, size: usize) -> Self {
        Self {
            compression_threshold: size,
            ..self
        }
    }
